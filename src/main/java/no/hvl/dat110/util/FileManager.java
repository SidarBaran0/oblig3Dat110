...
public class FileManager {
	...
	public void createReplicaFiles() {
		...
		for (int i = 0; i < Util.numReplicas; i++) {
			...
			BigInteger replicaHash = Hash.hashOf(fileName);
	...
	public int distributeReplicastoPeers() throws RemoteException {
		...
		NodeInterface prmNode = chordnode.findSuccessor(hash);

		for (int i = 0; i < numReplicas; i++) {

			BigInteger replicaId = replicafiles[i];
			NodeInterface successor = prmNode.findSuccessor(replicaId);
	...
	public Set<Message> requestActiveNodesForFile(String filename) throws RemoteException {
		...
		for (int i = 0; i < replicafiles.length; i++) {
			...
			// peer) of the file
			Message msg = successor.getFilesMetadata().get(replicafiles[i]);
			// save the metadata in the set activeNodesforFile.
			// dont save if msg == null
			if (msg != null) {
